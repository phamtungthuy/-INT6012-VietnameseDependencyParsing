# -*- coding: utf-8 -*-
"""
Triaffine Parser WITHOUT Word Embedding (Ablation Study)

Base: triaffine_parser.py
Ablation: Removes word embedding to measure its contribution
Input: BERT + POS Tag only (no n_words)
"""
...
class TriaffineNoWord(Model):
    """
    Triaffine Parser WITHOUT Word Embedding.
    Input: BERT + POS Tag only
    Purpose: Measure contribution of word embeddings
    """
    
    NAME = 'triaffine-noword'
    
    def __init__(
        self,
        n_words=None,  # Ignored in this variant
        n_tags=None,
        n_rels=None,
        n_embed=100,   # Ignored
        n_tag_embed=100,
        ...
        super(TriaffineNoWord, self).__init__()
        ...
        # NO Word Embedding (KEY DIFFERENCE)
        self.word_embed = None
        logger.info(f"[NoWord] Word embedding DISABLED for ablation study")
    ...
    def forward(self, words, feats, tags=None):
        ...
        # NO word embedding here (ablation)
