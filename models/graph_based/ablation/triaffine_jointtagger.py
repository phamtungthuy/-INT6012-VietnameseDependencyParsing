# -*- coding: utf-8 -*-
"""
Triaffine Parser with Joint POS Tagging (Ablation Study)

Base: triaffine_parser.py
Enhancement: Add a separate tagger branch like biaffine_parser_v3
Architecture:
    - Tagger Branch: BERT → BiLSTM → MLP → POS Tags
    - Parser Branch: (BERT ⊕ Tag Dense) → BiLSTM → MLP → Biaffine → Arcs/Rels
Purpose: Measure if adding a joint tagger helps or hurts parsing performance
"""
...
class TriaffineJointTagger(Model):
    """
    Triaffine Parser with Joint POS Tagger.
    
    Has two branches:
    1. Tagger Branch: Predicts POS tags from BERT
    2. Parser Branch: Uses BERT + predicted tag info for parsing
    """
    
    NAME = 'triaffine-jointtagger'
    ...
    def __init__(
        ...
        super(TriaffineJointTagger, self).__init__()
        ...
        logger.info(f"[JointTagger] Using Joint POS Tagger with {n_tags} tags")
