# -*- coding: utf-8 -*-
"""
Trainer for Triaffine Joint Tagger (Ablation Study)

Special trainer for the joint tagger variant since it has different
forward signature (returns s_tag, s_arc, s_rel).
"""
...
class JointTaggerTrainer:
    """Trainer for Triaffine with Joint POS Tagger."""
    ...
    def train(
        ...
        """Train the joint tagger parser."""
        os.makedirs(os.path.dirname(base_path), exist_ok=True)
        
        logger.info(f"[JointTagger] Building the fields")
        ...
        logger.info(f"[JointTagger] Vocab - Words: {n_words}, Tags: {n_tags}, Rels: {n_rels}")
        
        # Initialize model
        parser = TriaffineJointTagger(
            n_words=n_words,
            n_tags=n_tags,
            n_rels=n_rels,
        ...
        for epoch in range(1, max_epochs + 1):
            logger.info(f'[JointTagger] Epoch {epoch} / {max_epochs}:')
        ...
        logger.info(f"[JointTagger] Done. Best Dev: {best_metric}")
        logger.info(f"[JointTagger] Test: UAS={test_metric.uas:.2%}, LAS={test_metric.las:.2%}")
