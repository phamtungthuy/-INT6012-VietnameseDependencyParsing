# -*- coding: utf-8 -*-
"""
Triaffine Parser with Multi-Head Biaffine + Scalar Mix (Ablation Study)

Base: triaffine_parser.py (BiLSTM encoder)
Enhancement: Combines Multi-Head Biaffine Attention AND Scalar Mix (all 12 BERT layers)
"""
...
class MultiHeadBiaffine(nn.Module):
    """Multi-Head Biaffine Attention."""
    
    def __init__(self, n_in, n_heads=4, n_out=1, bias_x=True, bias_y=True):
        super(MultiHeadBiaffine, self).__init__()
        self.n_heads = n_heads
        self.n_out = n_out
        self.head_dim = n_in // n_heads
        
        self.proj_h = nn.Linear(n_in, n_in)
        self.proj_d = nn.Linear(n_in, n_in)
        
        self.heads = nn.ModuleList([
            Biaffine(n_in=self.head_dim, n_out=n_out, bias_x=bias_x, bias_y=bias_y)
        ...
        if n_out == 1:
            ...
            self.output = nn.Linear(n_heads * n_out, n_out)
    
    def forward(self, h_dep, h_head):
        batch, seq, _ = h_dep.shape
        
        h_dep = self.proj_d(h_dep).view(batch, seq, self.n_heads, self.head_dim)
        h_head = self.proj_h(h_head).view(batch, seq, self.n_heads, self.head_dim)
        
        scores = []
        for i, head in enumerate(self.heads):
            s = head(h_dep[:, :, i, :], h_head[:, :, i, :])
            scores.append(s)
        
        if self.n_out == 1:
            scores = torch.stack(scores, dim=-1)
            scores = self.output(scores).squeeze(-1)
        else:
...
class TriaffineMultiHeadScalarMix(Model):
    """
    Triaffine Parser with BOTH:
    - Multi-Head Biaffine Attention (4 heads)
    - Scalar Mix (all 12 BERT layers)
    """
    
    NAME = 'triaffine-multihead-scalarmix'
    ...
    def __init__(
        ...
        n_bert_layers=12,   # ALL LAYERS (Scalar Mix)
        bert_fine_tune=True,
        n_lstm_hidden=400,
        n_lstm_layers=3,
        lstm_dropout=0.33,
        n_mlp_arc=500,
        n_mlp_rel=100,
        n_biaffine_heads=4,  # Multi-Head
        mlp_dropout=0.33,
        feat_pad_index=0,
        transform=None,
        init_pre_train=False
    ):
        super(TriaffineMultiHeadScalarMix, self).__init__()
        
        self.bert_name = bert
        self.n_biaffine_heads = n_biaffine_heads
        ...
        self.args = {
            ...
            'n_mlp_arc': n_mlp_arc,
            'n_mlp_rel': n_mlp_rel,
            'n_biaffine_heads': n_biaffine_heads,
        ...
        logger.info(f"[MultiHead+ScalarMix] Using Scalar Mix with {n_bert_layers} BERT layers")
        ...
        # Multi-Head Biaffine Attention (ENHANCEMENT)
        logger.info(f"[MultiHead+ScalarMix] Using Multi-Head Biaffine with {n_biaffine_heads} heads")
        self.arc_attn = MultiHeadBiaffine(n_in=n_mlp_arc, n_heads=n_biaffine_heads, n_out=1, bias_x=True, bias_y=False)
